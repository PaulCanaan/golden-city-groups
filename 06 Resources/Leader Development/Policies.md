# Group Boundaries

## Document Status

**Approved minimum boundaries with church-wide policy implementation still open.** DEC-027, DEC-028, and DEC-033 govern the minimum leadership, care, safeguarding, participant-data, and formation boundaries reflected here. Additional operating details remain draft until approved. This document does not replace church-wide policy, legal guidance, mandated-reporting requirements, or pastoral authority.

## Theological Rationale

Group leadership is stewardship of people made in God's image. Boundaries serve truth, hospitality, holiness, safety, accountability, and the flourishing of Christ's church; they are not merely risk-management rules.

## Approved Minimum Boundaries and Draft Implementation

### Leadership and teaching

- Only explicitly appointed people are represented as group leaders.
- Leaders operate within approved role descriptions, church beliefs, and pastoral oversight.
- City Groups use approved sermon-based material and do not introduce unapproved speakers, curriculum, doctrinal claims, political campaigning, fundraising, or commercial promotion.
- Training attendance, Launch Team participation, or a Planning Center role does not constitute appointment.

### Participant care

- Leaders do not promise absolute confidentiality.
- Sensitive concerns move to approved pastoral, safeguarding, professional, or emergency support when required.
- Leaders do not act as licensed counselors, legal advisers, medical professionals, or sole crisis responders.
- Prayer and Scripture are never used to force disclosure, excuse harm, avoid reporting, or pressure a particular outcome.

### Safeguarding

- Groups involving minors, vulnerable people, childcare, transportation, overnight activity, or private homes follow the applicable church-approved safeguards before meeting.
- A background check alone is not treated as a complete safeguarding process.
- Suspected abuse, imminent danger, threats of harm, or other urgent concerns follow the approved reporting process immediately.

### Data and communication

- Planning Center access follows least privilege.
- Rosters, contact information, discernment records, background information, and identifiable pastoral concerns do not belong in this repository.
- Group chat, email, direct messages, event notes, and prayer-request sharing follow approved consent, visibility, moderation, and retention practices.

### Meetings and money

- Meeting information remains accurate and approved.
- No offering, fee, sale, solicitation, or financial assistance process is introduced through a group without church authorization.
- Leaders and hosts do not make facility, transportation, childcare, or financial commitments on behalf of the church without approval.

## Remaining Policy Work

The minimum boundaries above are approved. Russ directed staff to continue forming governing policies and confirm the primary and backup escalation contacts. The completed policy should identify:

- appointment, correction, pause, and removal authority
- primary and backup escalation contacts
- safeguarding and reporting procedures
- confidentiality and participant-data practices
- leader covenant or conduct standard
- group communication and meeting rules
- exceptions and approval authority

## Connections

- [[Leader Training]] — Leaders must understand approved boundaries before service.
- [[Prayer]] — Applies consent and confidentiality boundaries.
- [[Conflict]] — Applies correction and escalation boundaries.
- [[Planning Center Groups]] — Applies participant-data and access boundaries.
- [[Highlands Resource Adaptation]] — Explains why another church's honor code and safeguarding rules were not imported.
