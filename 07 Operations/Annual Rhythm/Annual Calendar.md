# Annual Calendar

## Document Status

**Working index.** Only dates supported by the active decision register or a dated readiness decision are confirmed. Future-season pages are planning scaffolds.

## Current Planning Horizon

- [[07 Operations/Annual Rhythm/2026/Fall and Winter 2026|Fall and Winter 2026]] — active pre-launch and launch-season planning
- [[07 Operations/Annual Rhythm/2027/Spring 2027|Spring 2027]] — blocked planning scaffold
- [[07 Operations/Annual Rhythm/2027/Summer 2027|Summer 2027]] — blocked planning scaffold

## Annual Review Pattern

For each ministry window:

1. Review the four environments and real pastoral needs.
2. Identify offerings to launch, develop, defer, continue, or close.
3. Confirm owners, leaders, locations, capacity, safeguarding, and participant paths.
4. Align the church calendar, Planning Center, Church Center, and communication.
5. Support leaders during the window.
6. Review formation, care, operations, and next steps afterward.

This pattern is a working recommendation adapted from GCC's launch roadmap and the transferable semester cycle recorded in [[Highlands Resource Adaptation]]. It does not establish dates or offerings.

## Connections

- [[07 Operations/Calendar|Calendar]] — Confirmed dates and pending milestones.
- [[Semester Planning]] — Repeatable planning process.
- [[07 Operations/Metrics|Metrics]] — Formation-aware review.
