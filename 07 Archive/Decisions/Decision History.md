# Decision History

## Archive Status

**Historical record through July 27, 2026.** This file preserves the full evidence, implications, boundaries, and open-question snapshot that previously occupied the active Decision Log. Use [[docs/decision-log|Active Decision Register]] for current governing summaries and the next decision queue.

Confirmed decisions in this archive remain governing unless a later decision explicitly supersedes them. Archiving reduces operational clutter; it does not revoke a decision.

## Original Purpose

The Decision Log is a governance tool, not a meeting transcript. It helps future leaders understand:

1. What has been decided?
2. What evidence supports that decision?
3. What ministry work is affected?
4. What still requires a decision?

Only record decisions that materially change the ministry's design, leadership, operations, curriculum, or governing documentation. Routine tasks, meeting notes, implementation progress, and minor content edits belong in their respective ministry or roadmap documents.

Optimize for long-term maintainability over completeness. Most entries should use the four-field default. Add context, rationale, or boundaries only when a foundational decision would otherwise be misunderstood.

## Statuses

### Decisions

- **Confirmed** — approved and governing
- **Proposed** — under discussion and not yet governing
- **Superseded** — replaced by a later decision

### Open Questions

- **Open** — a material decision is still required
- **Awaiting Pastoral Approval** — the matter cannot become governing without pastoral direction

## Confirmed Decisions

### DEC-001 — Public Church Launch Date

- **Decision:** Golden City Church is preparing to launch publicly on September 20, 2026.
- **Status:** Confirmed
- **Evidence:** Foundational project context recorded on 2026-07-21.
- **Implications:** Launch-critical ministry systems take priority over the ideal future model. This is the church's public launch date, not the start date for every Group or Class.

### DEC-002 — Central Formation Framework

- **Decision:** Belonging, Beholding, Becoming is the central Groups and Classes formation framework. The dimensions are mutually reinforcing rather than isolated program stages.
- **Status:** Confirmed
- **Evidence:** Foundational ministry context approved for this repository.
- **Implications:** The framework governs Groups, Classes, Leadership, Resources, participant pathways, and evaluation. Its public use remains unresolved in OQ-002.
- **Context:** The ministry needed a shared account of formation that would not reduce discipleship to attendance, information, or a rigid sequence.
- **Rationale:** The framework holds together covenant community, attention to Christ, and formation into Christlikeness and participation in God's mission.
- **Boundary:** It governs ministry design but does not establish a rigid participant pipeline.

### DEC-003 — Groups and Classes as One Ministry

- **Decision:** Groups and Classes are complementary environments within one ministry and should not operate as disconnected departments.
- **Status:** Confirmed
- **Evidence:** Foundational ministry context approved for this repository.
- **Implications:** Groups and Classes share church alignment, formation principles, participant pathways, and leadership development while retaining distinct purposes.
- **Context:** The repository contains separate Group and Class structures, but both serve one church mission and formation system.
- **Rationale:** Groups provide the primary relational and pastoral environment; Classes provide intentional biblical, theological, and ministry formation. Each strengthens the other.
- **Boundary:** Integration does not make one environment a prerequisite for the other or erase their different contributions.

### DEC-004 — Approved Ministry Purpose

- **Decision:** The approved Groups and Classes Ministry purpose is:

  > Groups provide the primary environment for Christ-centered community, pastoral care, prayer, mutual encouragement, and everyday discipleship. Classes provide intentional environments for deeper biblical, theological, and ministry formation, equipping people to live faithfully, serve the church, and help form others. Together they support Golden City Church in seeing people forge their faith, find freedom in Christ, and follow Jesus fully for the flourishing of the church and the witness of the gospel in the city.

- **Status:** Confirmed
- **Evidence:** Approved by Paul during repository development after review for alignment with Golden City Church.
- **Implications:** Ministry design, leadership, communication, resources, and evaluation should support this purpose and remain subordinate to Golden City Church's identity.
- **Context:** The ministry needed an approved purpose without creating an identity independent from Golden City Church.
- **Rationale:** The statement connects the distinct contributions of Groups and Classes to the church's confirmed mission language and gospel witness.
- **Boundary:** It does not authorize specific programs, schedules, practices, or launch offerings.

### DEC-005 — Context Architecture

- **Decision:** Repository work is governed by `AGENTS.md` and the context documents under `docs/`.
- **Status:** Confirmed
- **Evidence:** Context architecture approved during repository development.
- **Implications:** Contributors should review the governing context before substantive work and record new cross-cutting decisions here. These files govern repository contributions; Mission, Vision, Values, and the formation framework govern ministry design.

### DEC-006 — Groups and Classes Ministry Leadership

- **Decision:** Paul will lead Golden City Church's Groups and Classes Ministry.
- **Status:** Confirmed
- **Evidence:** Russ's launch announcement: “Paul will be leading our Groups and Classes ministry.”
- **Implications:** Ministry documents may identify Paul as the ministry lead. DEC-021 records his operational authority and the final readiness approval boundary; his formal title remains unresolved.

### DEC-007 — Planned Launch-Season Ministry Environments

- **Decision:** The launch-season plan includes City Groups, Men's Collective, Women's Collective, and Classes / Bible Club through Planted.
- **Status:** Confirmed
- **Evidence:** Russ introduced these environments with, “Here’s what we’re planning.”
- **Implications:** Groups, Classes, Leadership, Operations, safeguarding, and Communication may plan around these environments. Inclusion does not mean that every offering is staffed, located, approved, or launch-ready.

### DEC-008 — City Groups Model

- **Decision:** City Groups are planned as weekly, sermon-based small groups following the Sunday message. They may be co-ed or age-specific and may meet in homes or public spaces within communities Golden City Church is seeking to reach.
- **Status:** Confirmed
- **Evidence:** Russ's launch announcement describing the City Groups model and the goal of community close to home across the North Denver area.
- **Implications:** City Groups require coordination with Sunday teaching, discussion resources, leaders, hosts, locations, safeguarding, and weekly communication.
- **Context:** City Groups are the distributed small-group expression within the announced launch plan.
- **Rationale:** Russ stated the geographic aim as helping people “easily find community close to home.” No further rationale for the sermon-based model was stated.
- **Boundary:** Exact communities, leaders, locations, capacities, and age-specific expressions remain subject to readiness. North Denver is a directional field of ministry, not a formally defined or exclusive territory.

### DEC-009 — Men's and Women's Collective Model

- **Decision:** Men's Collective and Women's Collective are planned as larger, centrally located gatherings, similar to Team Nights, focused on community, discipleship, and semester-based study through a biblical book or curriculum.
- **Status:** Confirmed
- **Evidence:** Russ's launch announcement describing the Collective model.
- **Implications:** Collective planning affects curriculum, leaders, facilities, semester rhythms, and Communication. “Similar to Team Nights” does not require identical practices or schedules.

### DEC-010 — Planted as the Planned Bible Club Offering

- **Decision:** Planted is the identified Classes / Bible Club offering in the launch plan. Paul and Grant wrote it to help people deepen their understanding of Scripture, theology, and following Jesus.
- **Status:** Confirmed
- **Evidence:** Russ's launch announcement describing Planted as suitable for anyone wanting a stronger biblical foundation.
- **Implications:** Class planning may proceed around Planted. Authorship does not establish who will teach it, and the permanent relationship between the names Classes and Bible Club remains unresolved.

### DEC-011 — Launch Sign-Up and Ministry Window

- **Decision:** The stated goal is to offer sign-up opportunities on launch weekend and the following weekend and to run the announced ministry environments from early October through mid-December 2026.
- **Status:** Confirmed
- **Evidence:** Russ's launch announcement describing the two-weekend sign-up period and fall ministry window.
- **Implications:** Communication, intake, placement, leader preparation, and offering readiness must support the sequence from public launch, to sign-up, to an early-October ministry start. Exact dates remain open.

### DEC-012 — Initial Leader, Host, and Service Recruitment Path

- **Decision:** People interested in leading, hosting, or serving are being invited to pray and contact Paul or Russ. Potential homes and meeting locations are also being sought.
- **Status:** Confirmed
- **Evidence:** Russ's launch announcement inviting interested people to contact Paul or Russ.
- **Implications:** This is the initial launch-season invitation, not a permanent leadership-development process. Interest or contact does not constitute approval or appointment.
- **Boundary:** For the initial City Group leader pipeline, the more specific direction in DEC-018 governs. This entry may still describe general interest in hosting or serving but does not authorize public recruitment or appointment of City Group leaders.

### DEC-013 — Complementary Launch Ministry Architecture

- **Decision:** The launch plan uses three complementary forms: distributed City Groups, centralized Men's and Women's Collectives, and an intentional class environment through Planted.
- **Status:** Confirmed
- **Evidence:** Russ's descriptions of the setting, rhythm, and purpose of each launch environment.
- **Implications:** Groups, Classes, Leadership, facilities, curriculum, Communication, and Operations should preserve each environment's distinct contribution within one ministry.
- **Context:** The announcement describes multiple gathering forms rather than one model for every discipleship need.
- **Rationale:** Recording the relationship prevents the environments from becoming competing departments or interchangeable programs.
- **Boundary:** This architecture does not establish a required participant sequence, equal launch readiness, or a complete future ministry catalogue.

### DEC-014 — Public Groups Page Offering Set

- **Decision:** Golden City Church's public Groups page currently presents six offerings: Men's Collective, Planted Bible Study, Women's Collective, Run Club, Youth City Group, and Young Adults Group.
- **Status:** Confirmed
- **Evidence:** Golden City Church's official [Groups page](https://www.goldencity.church/groups), reviewed July 23, 2026.
- **Implications:** Public communication, calendar planning, registration, ministry ownership, safeguarding, and launch-readiness work must account for all six publicly listed offerings rather than only the four environments named in the earlier launch announcement.
- **Context:** Run Club, Youth City Group, and Young Adults Group were not included in DEC-007 or DEC-013 but are now represented publicly alongside Groups and Classes offerings.
- **Boundary:** Public listing confirms what the church is communicating; it does not establish that every offering belongs under Paul's Groups and Classes authority, has completed readiness review, or should be governed by the same leadership and safeguarding processes.
- **Subsequent direction:** DEC-022 defers Run Club, Youth City Group, and Young Adults Group from the initial Groups and Classes launch scope even while they remain part of this July 23 public-page snapshot.

### DEC-015 — Published Fall 2026 Rhythms and Locations

- **Decision:** The public Groups page currently communicates the following rhythms and locations:
  - Men's Collective: second and fourth Mondays at 7:00 p.m.; location to be determined.
  - Planted Bible Study: Tuesdays in fall 2026; location to be determined.
  - Women's Collective: first and third Mondays at 7:00 p.m.; I Can Workspace, 11878 Bradburn Boulevard, Westminster, Colorado 80031.
  - Run Club: Saturdays at 7:00 a.m., October 3 through December 12, 2026; McKay Lake Park in Broomfield.
  - Youth City Group: Sundays at 11:30 a.m.; check-in at the Legacy High School gym; intended for grades 6–12.
  - Young Adults Group: first Sunday of each month at 7:00 p.m.; location to be determined; intended for people in their twenties and thirties.
- **Status:** Confirmed
- **Evidence:** Golden City Church's official [Groups page](https://www.goldencity.church/groups), reviewed July 23, 2026.
- **Implications:** These details are the current public baseline for calendar, facilities, communication, leader preparation, and participant follow-up. Internal documents should not publish conflicting details without a recorded update.
- **Boundary:** “To be determined” locations remain unresolved, and public publication alone does not verify leader approval, capacity, safeguarding, childcare, supplies, or final launch readiness. Dates and locations should be reverified before derivative communication is released.

### DEC-016 — Public Church Center Enrollment Paths

- **Decision:** The public Groups page uses Church Center group pages as the enrollment path for Men's Collective, Women's Collective, and Planted Bible Study.
- **Status:** Confirmed
- **Evidence:** Links on Golden City Church's official [Groups page](https://www.goldencity.church/groups), reviewed July 23, 2026.
- **Implications:** Launch operations may treat Church Center as the current public enrollment platform for these three offerings and should define form ownership, notifications, placement, capacity handling, follow-up timing, and participant-data access around it.
- **Boundary:** This does not confirm a complete or tested participant journey. The public page does not provide a reliable offering-specific enrollment path for every listed offering, and participant-data governance remains unresolved in OQ-012.

### DEC-017 — Pre-Launch Gatherings and Launch Team

- **Decision:** Since March 2026, Golden City Church's Interest Socials, Dinner Parties, Worship Nights, local service opportunities, and Team Nights have intentionally formed a Launch Team of more than 50 committed volunteers who are preparing to become the church's initial ministry teams at public launch.
- **Status:** Confirmed
- **Evidence:** Approved ministry context provided for repository governance on July 23, 2026.
- **Implications:** Launch planning should begin with the relationships, service, commitment, and discernment already developing within the Launch Team rather than treating September 20 as the beginning of church community or volunteer formation.
- **Boundary:** Launch Team participation does not automatically appoint a person to leadership or confirm a specific ministry assignment.

### DEC-018 — Initial City Group Leadership Pipeline

- **Decision:** Initial City Group leaders will not be recruited from the general public. Russ intends for the first City Group leaders to be prayerfully identified, invited, discerned, and trained from within the Launch Team.
- **Status:** Confirmed
- **Evidence:** Approved ministry context provided for repository governance on July 23, 2026.
- **Implications:** Initial leader communication, sign-ups, discernment, training, and placement should be directed through the Launch Team and Team Nights. Public communication should not imply that any person may immediately become a City Group leader.
- **Context:** The Launch Team provides an existing relational setting in which commitment, character, service, alignment, and capacity can begin to be discerned before responsibility is entrusted.
- **Boundary:** This is the initial launch-season pipeline. It does not establish a permanent closed leadership system, identify any leader, replace pastoral approval, or define the full future leadership-development pathway.

### DEC-019 — Team Night City Group Leadership Preparation

- **Decision:** Team Nights on July 28, August 11, and August 25, 2026 will include dedicated time for City Group vision, leader sign-ups, leader discernment, placement conversations, and preparation for the September launch.
- **Status:** Confirmed
- **Evidence:** Approved ministry context provided for repository governance on July 23, 2026.
- **Implications:** The three Team Nights are the initial recruitment and discernment environment and must precede leader training, final group assignments, and commissioning in the launch sequence.
- **Boundary:** A Team Night sign-up expresses interest and availability; it does not constitute appointment. This decision does not establish the training schedule, commissioning date or form, number of groups, leader names, or final assignments.

### DEC-020 — Planning Center Groups Platform Context

- **Decision:** Planning Center Groups and Church Center are the current digital platform for group records and selected public enrollment paths. Launch operations should be designed around the platform's group records, membership, events, attendance, resources, communication, enrollment, and Church Center capabilities where those uses are approved.
- **Status:** Confirmed
- **Evidence:** Planning Center Groups export generated July 27, 2026; direct ministry instruction to integrate Planning Center Groups into the repository; and the existing public Church Center enrollment paths recorded in DEC-016.
- **Implications:** Operations should use a consistent Planning Center workflow for approved offerings and should avoid parallel rosters or participant-data stores in this repository. Platform configuration and ministry readiness must remain synchronized.
- **Boundary:** This confirms the platform context, not every current setting. It does not approve an offering, appoint a leader, authorize wider database access, establish participant-data policy, or resolve safeguarding, enrollment, or offering-specific readiness questions in OQ-003, OQ-006, OQ-007, or OQ-012.

### DEC-021 — Ministry Authority and Final Readiness Approval

- **Decision:** Paul is the operational point person for the Groups and Classes Ministry. He owns routine ministry design, coordination, leader support, calendar maintenance, Planning Center operation, ministry presentation, and leader-interest follow-up within confirmed church direction. Paul recommends an offering as ready; Russ, or a pastor explicitly designated by Russ, gives final readiness approval. Russ retains approval of doctrinal positions, safeguarding and participant-data policy, appointment or removal of primary leaders and instructors, material public ministry claims, significant budget or facility commitments, and exceptions to approved boundaries.
- **Status:** Confirmed
- **Evidence:** Decision 1 of the Pastoral Approval Brief was approved as written by Russ at the July 27, 2026 staff meeting. Russ further identified Paul as the ministry point person who will operate the ministry in Planning Center, present it at Team Nights and future regular weekends, and help potential future leaders sign up.
- **Implications:** Paul may lead routine implementation and Planning Center administration without returning every task to Russ. Final public readiness, primary appointments, and the named retained matters still require pastoral approval. A backup approver, exact Planning Center permission level, spending threshold, and emergency escalation path remain operational details to establish.
- **Boundary:** This decision does not give Paul independent doctrinal, safeguarding, participant-data, primary appointment/removal, major budget/facility, or exception authority. It does not resolve Paul's formal title.

### DEC-022 — Initial Launch Scope and Deferred Public Ideas

- **Decision:** The initial Groups and Classes launch scope remains City Groups, Men's Collective, Women's Collective, and Planted. Run Club, Youth City Group, and Young Adults Group are deferred ideas and do not need to begin with the Groups ministry.
- **Status:** Confirmed
- **Evidence:** Russ's direction on Decision 2 of the Pastoral Approval Brief at the July 27, 2026 staff meeting: “Run Club, Youth City Group, Young Adults Group are just ideas for now, we don't have to start the groups ministry with them.”
- **Implications:** Launch planning, staffing, safeguarding, Planning Center setup, and participant pathways should prioritize the four confirmed environments. Public website and Church Center presentation should not imply that the three deferred ideas are launch-ready. Ownership of each active launch offering still must be assigned before it opens.
- **Boundary:** Deferral does not cancel these ideas permanently or assign their future ownership. Any later activation requires an accountable owner and the appropriate readiness, safeguarding, communication, and participant-path review.

### DEC-023 — First Planted Instructor and Course Window

- **Decision:** Paul will lead the first Planted class after Golden City Church launches. The course is planned as a nine- or ten-week curriculum leading up to Christmas 2026.
- **Status:** Confirmed
- **Evidence:** Russ approved Paul as the Planted instructor in Decision 3 of the Pastoral Approval Brief at the July 27, 2026 staff meeting and gave the nine- or ten-week direction.
- **Implications:** Planted planning may proceed with Paul as instructor and a nine- or ten-week fall course window. Exact dates, location, capacity, materials, participant path, and final curriculum/readiness approval remain open.
- **Boundary:** This decision approves the first instructor and general length; it does not approve the broader leader, host, or instructor standard proposed in Decision 3 or confirm every operational detail of the class.

### DEC-024 — Distinct Formation Emphases Across Ministry Environments

- **Decision:** As an internal interpretive aid for ministry development, Golden City Church's three launch environments serve different ministry purposes through distinct formation emphases:
  - **City Groups:** approximately 80% fellowship and 20% structured discipleship—primarily shared life and relational time, with the weekly recap providing further discussion questions from the weekend message.
  - **Men's and Women's Collectives:** approximately 50% fellowship and 50% structured discipleship—balanced community and intentional biblical or formative engagement.
  - **Classes:** approximately 20% fellowship and 80% structured discipleship—an intentional space for both intellectual formation and spiritual formation.
- **Status:** Confirmed
- **Evidence:** Direction from Russ during the July 27, 2026 staff meeting while discussing Decisions 1–3 of the Pastoral Approval Brief.
- **Implications:** Internal ministry design, leader preparation, curriculum, gathering flow, and evaluation should preserve the distinct purpose of each environment rather than making every gathering feel like the same program. City Group weekly recaps should extend Sunday teaching without crowding out fellowship; Collectives should hold community and formation together; Classes should pursue understanding and spiritual response rather than information alone. Public communication should use qualitative descriptions rather than the ratios.
- **Context:** The ministry already distinguished distributed City Groups, centralized Collectives, and Classes. The percentages clarify their relative emphasis and help leaders design gatherings consistently.
- **Rationale:** People need complementary environments for friendship and belonging, balanced community and discipleship, and concentrated intellectual and spiritual formation.
- **Boundary:** The percentages are internal ministry-development heuristics, not timed quotas, numerical performance targets, public Groups Ministry language, or a claim that fellowship is outside discipleship. All three environments should include belonging, beholding, and becoming; they differ in emphasis, not in whether formation occurs.

The open questions below are preserved as a July 27, 2026 snapshot. The current decision queue is maintained in [[docs/staff-decision-brief|Staff Decision Brief]].

## Open Questions

### OQ-002 — Public Formation Language

- **Status:** Awaiting Pastoral Approval
- **Question:** Will Belonging, Beholding, Becoming be public-facing language or primarily an internal framework?

### OQ-003 — Launch-Ready Group Expressions

- **Status:** Open
- **Question:** Of the four environments prioritized in DEC-022, which specific offerings have completed launch-readiness review, and which existing geographic City Group pages are active, tentative, future, or archival?

### OQ-004 — Planted Curriculum Readiness

- **Status:** Awaiting Pastoral Approval
- **Question:** Paul is confirmed as the first instructor under DEC-023. What pastoral, theological, and launch-readiness review must the Planted curriculum complete before the class begins?

### OQ-005 — Leadership Standards and Discernment

- **Status:** Awaiting Pastoral Approval
- **Question:** Within the confirmed Launch Team and Team Night pipeline, what discernment, qualifications, training, authority, reporting, appointment, correction, and removal processes govern leaders, hosts, coaches, and instructors?

### OQ-006 — Pastoral Care and Safeguarding

- **Status:** Awaiting Pastoral Approval
- **Question:** Which church-wide policies govern confidentiality, reporting, abuse, crises, minors, vulnerable people, childcare, homes, background checks, referrals, and the announced meeting environments?

### OQ-007 — Sign-Up and Placement

- **Status:** Open
- **Question:** Who owns the Church Center enrollment paths identified in DEC-016, submission notifications, timely follow-up, placement, capacity or waitlist handling, and the enrollment path for offerings without a reliable offering-specific link?

### OQ-008 — Mission Language in Practice

- **Status:** Awaiting Pastoral Approval
- **Question:** How should Groups and Classes teach and practice freedom in Christ, flourishing, service, and generosity?

### OQ-009 — Holy Spirit Practices

- **Status:** Awaiting Pastoral Approval
- **Question:** What practices related to prayer, spiritual gifts, discernment, and ministry in the Spirit should be expected in Groups, Classes, and leader training?

### OQ-010 — Fall Launch Operations

- **Status:** Open
- **Question:** Paul owns routine launch coordination under DEC-021. Which published details in DEC-015 are final, who is the accountable leader or owner for each offering, and what training dates, commissioning practice, group assignments, capacities, budget, childcare, supplies, safeguarding requirements, backups, and contingencies will govern each offering?

### OQ-011 — Planted Operational Readiness

- **Status:** Open
- **Question:** Paul is confirmed as instructor for a nine- or ten-week course leading to Christmas. Which dates and location will Planted use, and what capacity, materials, support roles, and Church Center registration requirements will govern it?

### OQ-012 — Participant Data Governance

- **Status:** Awaiting Pastoral Approval
- **Question:** What participant information may be collected, where may it be stored, who may access it, how long may it be retained, and what privacy and consent practices are required?

### OQ-014 — Classes and Bible Club Naming

- **Status:** Awaiting Pastoral Approval
- **Question:** Is Bible Club the public name for the Classes ministry, one expression within Classes, or a temporary alternative name?

### OQ-016 — Public Groups Page Participant Paths

- **Status:** Open
- **Question:** What approved destination should the public “Find a Group” and “Lead a Group” calls to action use, what participant path should each launch offering provide, and how should the three deferred ideas in DEC-022 be removed or clearly represented as future?

## Entry Templates

### Level 1 — Default Decision

```markdown
### DEC-XXX — Title

- **Decision:** What was decided?
- **Status:** Confirmed | Proposed | Superseded
- **Evidence:** What supports the decision?
- **Implications:** What ministry work is affected?
```

### Level 2 — Foundational Decision

Add only the fields needed to prevent future misunderstanding:

```markdown
- **Context:** What situation made the decision necessary?
- **Rationale:** Why was this direction chosen?
- **Boundary:** What does the decision not establish or authorize?
```

Optional metadata such as a date, related files, or a superseded decision may be added when materially useful.

### Open Question

```markdown
### OQ-XXX — Title

- **Status:** Open | Awaiting Pastoral Approval
- **Question:** What still requires a decision?
```
